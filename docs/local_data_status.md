...
The first real-data baseline is profile-based retrieval over CPJUMP1 morphology profiles. Raw JUMP images are not required yet.
...
DINOv3 is a later optional image/composite baseline after local image or composite handling exists. RAG is later query and explanation support, not the first priority for this Phase 2 data path.
